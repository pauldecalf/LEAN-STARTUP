...
export class AppController {
  ...
  async joinFamily(
    ...
    try {
      ...
      return response
        .status(200)
        .json({ message: 'Rejoint la famille avec succès' });
    } catch (error) {
  ...
  async register(
    ...
    try {
      ...
      return response
        ...
        .json({ message: 'Une erreur est survenue lors de votre inscription' });
    }
  }
    ...
    try {
      ...
      if (!payload) {
        return response
          ...
          .json({ message: 'Token Google invalide' });
      }
      ...
      return response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
        message:
          'Une erreur est survenue lors de votre inscription avec Google',
      });
    }
  }
  ...
  async login(
    ...
    try {
      ...
      return response
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: 'Une erreur est survenue lors de votre connexion' });
    }
  }
  ...
  async loginWithGoogle(
    ...
    try {
      ...
      return response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
        message: 'Une erreur est survenue lors de votre connexion avec Google',
      });
    }
  }
  ...
  @Post('/create-family')
  async create(
    @Body() { name, createdBy }: { name: string; createdBy: string },
    @Res() response: Response,
  ) {
    try {
      // Vérifiez si une famille existe déjà pour cet utilisateur
      const existingFamily =
        await this.famillesService.findByCreatedBy(createdBy);
      if (existingFamily) {
  ...
  async getDashboardPost(
    ...
    try {
      ...
      const usersInFamily = await this.usersService.findByFamilleId(
        user.idFamille,
      );
  ...
  @Post('/update-role')
  async updateRole(
    @Body() { email, role }: { email: string; role: string },
    @Res() response: Response,
  ) {
    try {
  ...
  async getProfilPost(@Body() body: { email: string }, @Res() res: Response) {
    ...
    try {
      ...
      if (!family) {
        return res
          .status(HttpStatus.NOT_FOUND)
          .json({ message: 'Famille non trouvée' });
      }
      // Retourner un objet JSON complet
      return res.json({
        nomFamily: family.nom,
        role: user.role,
        prenom: user.prenom,
        email: user.email,
        imageProfil: user.imgProfil,
      });
    } catch (error) {
      return res
        .status(HttpStatus.INTERNAL_SERVER_ERROR)
