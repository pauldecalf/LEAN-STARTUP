...
export class PrisedeconnaissanceService {
  constructor(
    @InjectModel('Prisedeconnaissance')
    private readonly prisedeconnaissanceModel: Model<Prisedeconnaissance>,
  ) {}

  async create(
    createPrisedeconnaissanceDto: CreatePrisedeconnaissanceDto,
  ): Promise<Prisedeconnaissance> {
    const createdPrisedeconnaissance = new this.prisedeconnaissanceModel(
      createPrisedeconnaissanceDto,
    );
